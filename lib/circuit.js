...
const PERMITTED_NUMBER = Symbol('permitted-number');
const HALF_OPEN_STAGE = Symbol('half-open-stage');
 ...
 * @param {Number} options.permittedNumberOfCallsInHalfOpenState configures the
 * number of permitted calls when the CircuitBreaker is half open. Default: 1
 * @param {Array} options.halfOpenStages configures the half-open transition
 * stags. In half-open state, configure multiple ratio stages to determin
 * how much percentage of traffic can be used to test cicruit. Default: [1]
 * @param {boolean} options.enabledHalfOpenDebug open this to help debugging
 * half open stage. Default: false.
 *
...
class CircuitBreaker extends EventEmitter {
  ...
  */
  ...
  constructor (action, options = {}) {
    ...
    this.enabledHalfOpenDebug = options.enabledHalfOpenDebug || false;
    ...
    // normalize halfOpenStages
    if (!Array.isArray(options.halfOpenStages)) {
      console.warn('options.halfOpenStages should be an array of number. Fallback to default.');
      this.options.halfOpenStages = [1];
    } else {
      if (options.halfOpenStages.length === 0) {
        console.warn('options.halfOpenStages should at least contain 1 value. Fallback to default.');
        this.options.halfOpenStages = [1];
      }
      const max = Math.max.apply(null, options.halfOpenStages);
      const min = Math.min.apply(null, options.halfOpenStages);
      if (min <= 0) {
        console.warn('value of options.halfOpenStages should strictly greater than 0. Fallback to default.');
        this.options.halfOpenStages = [1];
      }
      if (max !== 1) {
        console.warn('max value of options.halfOpenStages should equal to 1. Fallback to default.');
        this.options.halfOpenStages = [1];
      }
      this.options.halfOpenStages =
        this.options.halfOpenStages.sort((o1, o2) => o1 - o2);
    }

    this.options.permittedNumberOfCallsInHalfOpenState =
      Number.isInteger(options.permittedNumberOfCallsInHalfOpenState)
        ? options.permittedNumberOfCallsInHalfOpenState
        : 1;
    ...
    this[PERMITTED_NUMBER] = 0;
    this[HALF_OPEN_STAGE] = 0;
    ...
    this.on('halfOpenFailure', _ => {
      this.halfOpenStats.failures += 1;
      if (!this.halfOpenStatsCheckSum) return;
      if (
        this[PERMITTED_NUMBER] >=
        this.options.permittedNumberOfCallsInHalfOpenState
      ) {
        if (this.halfOpenErrorRate < this.options.errorThresholdPercentage) {
          if (!this.getNextStages()) {
            this.close();
          } else {
            this[PERMITTED_NUMBER] = 0;
          }
    ...
    this.on('halfOpenFire', _ => {
      this[PERMITTED_NUMBER] += 1;
      this.halfOpenStats.fires += 1;
    });
  ...
  close () {
    if (this[STATE] !== CLOSED) {
      ...
      this[STATE] = CLOSED;
      this[PENDING_CLOSE] = false;
      this.resetHalfOpenStatus();
      this.resetHalfOpenStats();
      /**
  ...
  get halfOpenStage () {
    return this.options.halfOpenStages[this[HALF_OPEN_STAGE]];
  }

  get halfOpenErrorRate () {
    return this.halfOpenStats.failures / this.halfOpenStats.fires * 100;
  }

  get halfOpenStatsCheckSum () {
    return this.halfOpenStats.failures + this.halfOpenStats.success
     === this.halfOpenStats.fires
     && this.halfOpenStats.fires > 0;
  }

  getNextStages () {
    if (this[HALF_OPEN_STAGE] === this.options.halfOpenStages.length - 1) {
      return false;
    } else {
      this[HALF_OPEN_STAGE] = this[HALF_OPEN_STAGE] + 1;
      this.resetHalfOpenStats();
      return true;
    }
  }
  ...
  call (context, ...rest) {
    ...
    const halfOpenRandom = Math.random();

    if(this.halfOpen && halfOpenRandom <= this.halfOpenStage) {
      if (
        this[PERMITTED_NUMBER] >=
        this.options.permittedNumberOfCallsInHalfOpenState
      ) {
        const error = buildError(
          `Breaker get more than ${
            this.options.permittedNumberOfCallsInHalfOpenState
          } requests in ${this.halfOpenStage} half open stage`
        );
        return fallback(this, error, args) ||
          Promise.reject(error);
      } else {
        this.emit('halfOpenFire');
      }
    }
    if (this.halfOpen && halfOpenRandom > this.halfOpenStage) {
      console.warn(halfOpenRandom, this.halfOpenStage);
      const error = buildError(`Breaker is half-open, and this request cannot pass through by probability: ${this.halfOpenStage}.`, 'EHALFOPENBREAKER');

      if (this.enabledHalfOpenDebug) {
        console.warn(`Breaker is half-open, and this request cannot pass through by probability: ${this.halfOpenStage}.`, 'EHALFOPENBREAKER');
      }
...
function fail (circuit, err, args, latency) {
  ...
  if(circuit.halfOpen) {
    circuit.emit('halfOpenFailure', err, latency, args);
    return;
  }
