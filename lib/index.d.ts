...
declare class CircuitBreaker<TI extends unknown[] = unknown[], TR = unknown> extends EventEmitter {
    ...
    readonly halfOpenStats: CircuitBreaker.HalfOpenStats
    readonly halfOpenStage: number
    readonly halfOpenErrorRate: number
    readonly halfOpenStatsCheckSum: boolean
    ...
    on(event: 'halfOpenFire', listener: () => void): this;
    on(event: 'halfOpenSuccess', listener: () => void): this;
    on(event: 'halfOpenFailure', listener: (err: Error, latencyMs: number, args: TI) => void): this;
...
declare namespace CircuitBreaker {
    interface Options {
        ...
        /**
         * Configure the number of permitted calls when the CircuitBreaker is half open. Default: 1 
         */
